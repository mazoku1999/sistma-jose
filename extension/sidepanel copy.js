...
function wireHeaderBack(handler) {
    ...
    if (!btn) return;
    btn.onclick = handler;
    btn.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') handler(e); };
                        ...
                        const updateDoc = (doc) => {
                            ...
                            rows.forEach((tr, idx) => {
                                try {
                                    ...
                                    // Prefix match for categorized subjects (e.g., 'comunicacion y lenguajes' vs full base)
...
async function fillRandomOnPage() {
    try {
        await chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
            const tabId = tabs[0]?.id;
            if (!tabId) return;
            await chrome.scripting.executeScript({
                ...
                func: () => {
                    ...
                    const updateDoc = (doc) => {
                        const inputs = Array.from(doc.querySelectorAll('td[data-title*="Trimestre"] input.nota[id$="-7"], input.nota[id$="-7"][name="nota[]"]'));
...
<div class="sp-card">
  <div class="sp-toolbar">
    ...
    <div class="sp-row" style="gap:8px; flex-wrap:wrap">
      <button id="injectNotes" class="tw-btn tw-btn-primary">Cargar notas en esta página</button>
      <button id="fillRandom" class="tw-btn">Cargar random</button>
    </div>
  </div>
  <div class="sp-summary">${chips}</div>
    ...
    function bindRows() {
        container.querySelectorAll('[data-estudiante-id]').forEach(row => {
            row.addEventListener('click', () => {
                ...
                // Wire buttons in detail toolbar
                ...
                const fillRandomBtn = container.querySelector('#fillRandom');
                if (fillRandomBtn) fillRandomBtn.addEventListener('click', async () => { await fillRandomOnPage(); });
                // Cargar solo cuando el usuario lo solicite con el botón
                wireHeaderBack(() => {
                    setHeaderBackVisible(false);
